...
const SUPABASE_URL = "https://toyhvsnagunxxtlettrq.supabase.co/rest/v1/"; // REEMPLAZAR
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRveWh2c25hZ3VueHh0bGV0dHJxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk3NDc4MTksImV4cCI6MjEwNTMyMzgxOX0.e_HS5qmM7hOk9k41vBTuQbLtAOrXpakEk9m8-LAkf6Q"; // REEMPLAZAR
...
async function crearReserva() {
  ...
  if (error) {
    alert("No se pudo realizar la reserva. El horario/máquina seleccionado ya se encuentra ocupado.");
  } else {
...
async function cargarMisReservas() {
  ...
  reservas.forEach(r => {
    ...
    div.innerHTML = `
      <h4>${r.cursos.nombre} - RLAB ${r.maquinas.numero_rlab}</h4>
...
async function guardarCurso(e) {
  ...
  const { error } = await supabase.from("cursos").insert({ nombre, codigo });
...
async function asignarAlumnoCurso(e) {
  ...
  const { error } = await supabase.from("curso_estudiantes").insert({ curso_id, usuario_id: profile.id });
...
async function guardarPerfil() {
  ...
  if (nombre) {
    await supabase.from("profiles").update({ nombre_completo: nombre }).eq("id", currentUser.id);
  }
  if (pass) {
